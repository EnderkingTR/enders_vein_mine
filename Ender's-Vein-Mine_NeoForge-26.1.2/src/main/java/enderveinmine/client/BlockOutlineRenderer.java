...
public class BlockOutlineRenderer {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent.AfterLevel event) {
        ...
        for (BlockPos pos : cachedBlocks) {
            ...
            boolean drawDown = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.below()), Direction.DOWN);
            boolean drawUp = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.above()), Direction.UP);
            boolean drawNorth = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.north()), Direction.NORTH);
            boolean drawSouth = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.south()), Direction.SOUTH);
            boolean drawWest = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.west()), Direction.WEST);
            boolean drawEast = Block.shouldRenderFace(level, pos, state, level.getBlockState(pos.east()), Direction.EAST);

            if (!drawDown && !drawUp && !drawNorth && !drawSouth && !drawWest && !drawEast) continue;
            ...
            shape.forAllBoxes((minX, minY, minZ, maxX, maxY, maxZ) -> {
                ...
                // Draw Edges (Lines)
                // Bottom edges
                if (drawDown || drawNorth) drawLine(consumer, pose, fMinX, fMinY, fMinZ, fMaxX, fMinY, fMinZ, red, green, blue, alpha);
                if (drawDown || drawSouth) drawLine(consumer, pose, fMinX, fMinY, fMaxZ, fMaxX, fMinY, fMaxZ, red, green, blue, alpha);
                if (drawDown || drawWest) drawLine(consumer, pose, fMinX, fMinY, fMinZ, fMinX, fMinY, fMaxZ, red, green, blue, alpha);
                if (drawDown || drawEast) drawLine(consumer, pose, fMaxX, fMinY, fMinZ, fMaxX, fMinY, fMaxZ, red, green, blue, alpha);
                // Top edges
                if (drawUp || drawNorth) drawLine(consumer, pose, fMinX, fMaxY, fMinZ, fMaxX, fMaxY, fMinZ, red, green, blue, alpha);
                if (drawUp || drawSouth) drawLine(consumer, pose, fMinX, fMaxY, fMaxZ, fMaxX, fMaxY, fMaxZ, red, green, blue, alpha);
                if (drawUp || drawWest) drawLine(consumer, pose, fMinX, fMaxY, fMinZ, fMinX, fMaxY, fMaxZ, red, green, blue, alpha);
                if (drawUp || drawEast) drawLine(consumer, pose, fMaxX, fMaxY, fMinZ, fMaxX, fMaxY, fMaxZ, red, green, blue, alpha);
                // Vertical edges
                if (drawNorth || drawWest) drawLine(consumer, pose, fMinX, fMinY, fMinZ, fMinX, fMaxY, fMinZ, red, green, blue, alpha);
                if (drawNorth || drawEast) drawLine(consumer, pose, fMaxX, fMinY, fMinZ, fMaxX, fMaxY, fMinZ, red, green, blue, alpha);
                if (drawSouth || drawWest) drawLine(consumer, pose, fMinX, fMinY, fMaxZ, fMinX, fMaxY, fMaxZ, red, green, blue, alpha);
                if (drawSouth || drawEast) drawLine(consumer, pose, fMaxX, fMinY, fMaxZ, fMaxX, fMaxY, fMaxZ, red, green, blue, alpha);
            });
        }

        // Draw filled quads
        VertexConsumer quadConsumer = bufferSource.getBuffer(RenderTypes.debugFilledBox());
        for (BlockPos pos : cachedBlocks) {
            BlockState state = level.getBlockState(pos);
            if (state.isAir()) continue;
    ...
    private static void drawLine(VertexConsumer consumer, PoseStack.Pose pose, float x1, float y1, float z1, float x2, float y2, float z2, float red, float green, float blue, float alpha) {
        consumer.addVertex(pose, x1, y1, z1).setColor(red, green, blue, alpha).setNormal(0, 1, 0).setLineWidth(2.0f);
        consumer.addVertex(pose, x2, y2, z2).setColor(red, green, blue, alpha).setNormal(0, 1, 0).setLineWidth(2.0f);
    }
