...
public class BlockBreakHandler {
    ...
    public static void register() {
        PlayerBlockBreakEvents.BEFORE.register((level, player, pos, state, blockEntity) -> {
            if (level.isClientSide() || !(player instanceof ServerPlayer serverPlayer)) return true;
            ...
            try {
                ...
                if (breakBlock((ServerLevel) level, serverPlayer, pos, state, tool, damageTool, directDrop)) {
                    ...
                    if ("SHAPED".equals(shapeModeStr)) {
                        handleShapedMine((ServerLevel) level, serverPlayer, pos, state, customShape, maxBlocks, damageTool, directDrop, preventToolBreak, hungerDrain);
                    } else if ("SHAPELESS".equals(shapeModeStr)) {
                        handleShapelessMine((ServerLevel) level, serverPlayer, pos, state, maxBlocks, damageTool, directDrop, preventToolBreak, hungerDrain);
                    } else {
                        handlePresetMine((ServerLevel) level, serverPlayer, pos, state, shapeModeStr, maxBlocks, damageTool, directDrop, preventToolBreak, hungerDrain);
                    }
                    return false;
                }
            } finally {
                    ...
                    for (int z = -1; z <= 1; z++) {
                        ...
                        if (BlockUtils.isSameBlockGroup(startState, state)) {
                            ...
                            if (breakBlock(level, player, offset, state, tool, damageTool, directDrop)) {
                                ...
                                brokenCount++;
                                queue.add(offset);
                                if (checkTool && tool.isEmpty()) break; // Tool broke
                            }
                        }
                    }
