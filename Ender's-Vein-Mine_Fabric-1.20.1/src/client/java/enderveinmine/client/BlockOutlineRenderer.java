...
public class BlockOutlineRenderer implements WorldRenderEvents.Last {
    ...
    public void onLast(WorldRenderContext context) {
        ...
        // Draw lines first
        com.mojang.blaze3d.vertex.VertexConsumer consumer = context.consumers().getBuffer(net.minecraft.client.renderer.RenderType.lines());
        for (BlockPos pos : cachedBlocks) {
            ...
            shape.forAllBoxes((minX, minY, minZ, maxX, maxY, maxZ) -> {
                ...
                org.joml.Matrix4f p = pose.pose();
                org.joml.Matrix3f n = pose.normal();
                
                // Draw lines of the box
                consumer.vertex(p, fMinX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 1, 0, 0).endVertex();
                consumer.vertex(p, fMaxX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 1, 0, 0).endVertex();
                consumer.vertex(p, fMinX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 1, 0).endVertex();
                consumer.vertex(p, fMinX, fMaxY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 1, 0).endVertex();
                consumer.vertex(p, fMinX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 0, 1).endVertex();
                consumer.vertex(p, fMinX, fMinY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, 0, 1).endVertex();
                
                consumer.vertex(p, fMaxX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, -1, 0, 0).endVertex();
                consumer.vertex(p, fMinX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, -1, 0, 0).endVertex();
                consumer.vertex(p, fMaxX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, -1, 0).endVertex();
                consumer.vertex(p, fMaxX, fMinY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, -1, 0).endVertex();
                consumer.vertex(p, fMaxX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, 0, -1).endVertex();
                consumer.vertex(p, fMaxX, fMaxY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 0, -1).endVertex();
                
                consumer.vertex(p, fMinX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, -1, 0).endVertex();
                consumer.vertex(p, fMinX, fMinY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, -1, 0).endVertex();
                consumer.vertex(p, fMinX, fMaxY, fMaxZ).color(red, green, blue, alpha).normal(n, 1, 0, 0).endVertex();
                consumer.vertex(p, fMinX, fMaxY, fMinZ).color(red, green, blue, alpha).normal(n, 1, 0, 0).endVertex();
                
                consumer.vertex(p, fMaxX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 1, 0).endVertex();
                consumer.vertex(p, fMaxX, fMaxY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 1, 0).endVertex();
                consumer.vertex(p, fMaxX, fMinY, fMinZ).color(red, green, blue, alpha).normal(n, 0, 0, 1).endVertex();
                consumer.vertex(p, fMaxX, fMinY, fMaxZ).color(red, green, blue, alpha).normal(n, 0, 0, 1).endVertex();
            });
        }
        
        // Draw filled quads second
        com.mojang.blaze3d.vertex.VertexConsumer quadConsumer = context.consumers().getBuffer(net.minecraft.client.renderer.RenderType.debugFilledBox());
        for (BlockPos pos : cachedBlocks) {
            BlockState state = level.getBlockState(pos);
            if (state.isAir()) continue;
            ...
            shape.forAllBoxes((minX, minY, minZ, maxX, maxY, maxZ) -> {
                ...
                net.minecraft.client.renderer.LevelRenderer.addChainedFilledBoxVertices(poseStack, quadConsumer, fMinX, fMinY, fMinZ, fMaxX, fMaxY, fMaxZ, red, green, blue, fillAlpha);
            });
        }
        poseStack.popPose();
    }
